(()=>{
  ...
  const join=async(name,count)=>{
    const parts=await Promise.all(Array.from({length:count},(_,i)=>fetch(new URL(`./media/${name}-${i}.txt`,base),{cache:'force-cache'}).then(r=>{if(!r.ok)throw new Error(`${name}-${i} unavailable`);return r.text();})));
  ...
  })().catch(err=>{console.warn('IALS premium media fallback active',err);return null;});
})();
