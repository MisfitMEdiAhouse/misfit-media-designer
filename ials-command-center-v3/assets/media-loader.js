(()=>{
  ...
  const logoUrl=new URL('./ials-warbird.svg?v=captain-cigar-final',base).href;

  /*
   * The logo is a normal static asset. It must never depend on the hero,
   * chunk reconstruction, canvas processing, or a successful async repair.
   */
  ...
  style.dataset.ialsFix='static-logo-mobile-layout';
  style.textContent=`
    ...
    img[data-ials-logo],img[src*="ials-warbird.svg"]{
      ...
      filter:drop-shadow(0 9px 20px rgba(0,0,0,.78))!important;
    }
    ...
    @media(max-width:760px){
      ...
      html.ials-hero-ready .hero:before{
        ...
        aspect-ratio:16/9!important;background-image:linear-gradient(0deg,rgba(1,5,8,.3),transparent 48%),var(--ials-hero-image)!important;
      ...
      .footer{padding:28px 20px 24px!important}
      .footer-grid{text-align:center!important;gap:16px!important}
      ...
      .login-card{margin:16px 14px!important;padding:24px 20px!important}
  ...
  const join=async(name,count)=>{
    const parts=await Promise.all(Array.from({length:count},(_,i)=>
      fetch(new URL(`./media/${name}-${i}.txt`,base),{cache:'no-store'}).then(r=>{
  ...
  window.IALSMediaReady=loadHero();
